...
* `env` コマンドで bash スクリプトを実行でき、`sort`, `awk` などの基本ツールを使える
    ...
    Windows では [Git for Windows](https://gitforwindows.org) ([MSYS2](https://www.msys2.org)) の環境で実行できます。`env.exe` のあるディレクトリ（例えば `C:\opt\PortableGit\usr\bin` や `C:\Program Files\Git\usr\bin`）が Windows のコマンド検索パス（`Path`）に含まれている必要があります。
...
### install.sh を使う方法
インストール先（例えば `~/bin`）を引数に指定して `install.sh` を実行します。Windows では bash (Git Bash) のコマンド行で実行します。
```bash
$ bash install.sh ~/bin
```
...
`-l`
: ファイルをコピーするのではなく、インストール先から作業ディレクトリのファイルへのシンボリック・リンクを作成します。
...
## アンインストール
Windows では必要に応じてスクリプトのインストール先を Windows のコマンド検索パス（`Path`）から削除します。

以下のどちらかの方法でアンインストールします。

### uninstall.sh を使う方法
インストール先（例えば `~/bin`）を引数に指定して `uninstall.sh` を実行します。
...
### VS Code, MPE
VS Code のコードチャンクから bash スクリプトを実行できることを確認します。
...
4. `Run Code Chunk` で Code chunk 2 を実行します。
    Code chunk 2 は `env` コマンドで `./test.sh` を実行します。
...
3. `Run Code Chunk` で Code chunk 1 または Code chunk 2 を実行します。
    ...
    `asyco` または `mepoco` のバージョンが表示されることを確認します。もし表示されなければ、インストール先（例えば `$HOME/bin`）が bash のコマンド検索パス（`PATH`）に含まれることを確認します。Windows ではインストール先（例えば `%USERPROFILE%\bin`）が Windows のコマンド検索パス（`Path`）に含まれることを確認します。
