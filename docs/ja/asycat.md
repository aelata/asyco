...
`asycat` を使うと、[Asymptote](https://asymptote.sourceforge.io) または [MetaPost](https://tug.org/metapost.html) のファイルから、コードと図を横に並べたマークダウンを生成できます。

![asycat が生成するマークダウンの Visual Studio Code でのプレビュー](color.png "asycat が生成するマークダウンの Visual Studio Code でのプレビュー"){width=65% style="display:block;margin:auto;"}

マークダウンは、[Visual Studio Code (VS Code)](https://code.visualstudio.com) と、ブラウザや [pandoc](https://www.pandoc.org) を経由することで PDF ファイルに変換できます。`asycat` はコードチャンクからは呼び出せず、コマンド行から実行する必要があります。
...
## 使用法
### マークダウンファイルの作成
Asymptote や MetaPost のファイルを引数に指定して `asycat` を実行し、出力されるマークダウンをファイルにリダイレクトします。
...
`--fig-width=WIDTH`
: 横並びの配置（`-P E` または `-P W`）の場合に、図の幅を *`WIDTH`* に設定します。省略時値は `30%` です。

`--fixed-layout`
: 横並びの配置（`-P E` または `-P W`）の場合に、`table-layout` を `fixed` に設定します。
...
`--`
: オプションの終了を示します。この後はファイル名またはディレクトリ名として扱われます。
...
以下のクラスは、横並びの配置（`-P E` または `-P W`）の場合に有効です。
...
## 使用例
### 複数のディレクトリの指定
ディレクトリは `--` の後または最後の引数として指定する必要があります。

```bash
$ asycat -- tests docs/color > output.md
```

マークダウン文書 `output.md` を、ディレクトリ `tests` と `docs/color` の下の、拡張子が `.asy` または `.mp` のファイルから作成します。
...
図の表示部のコードチャンクのオプションは次のようになり、`args` に `asyco` のオプション `--cd=tests` が追加されます。
...
図の表示部のコードチャンクのオプションは次のようになり、`args` に `--cd` オプションは追加されません。
...
この場合 `rgb.md` と `cmyk.md` の両方で、`id` が `fig-1` のコードチャンクを参照します。
...
この場合 `id` が `fig-c-1` のコードチャンクが生成されます。
